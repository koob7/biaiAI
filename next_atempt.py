...
for i in range(10):
    train_df = train_df._append(df[df.category == i].iloc[:500, :])
...
model.compile(loss='binary_crossentropy', optimizer=optimizers.SGD(learning_rate=1e-4, momentum=0.9), metrics=['accuracy'])

model.summary()

model.save('animal_classifier_2.keras')
...
print("CNN: Epochs={0:d}, Train accuracy={1:.5f}, Validation accuracy={2:.5f}".format(epochs,
